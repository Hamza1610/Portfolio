        ...
        <Card style={{ width: '12rem', height: '20rem', fontSize: '.5rem'}} className='custon-card m-0 bg-secondary fs-5' border='primary'>
            <Card.Body>
                ...
                <div style={{ width: '100%', height:'30px', display: 'flex', gap: '5px'}}>
                    <Link onClick={() => navigator.clipboard.writeText( baseUrl + 'profile/' + profile._id)}  class='btn btn-primary' style={{ width: '50%',  height: 'auto', fontSize: '.7rem', color: 'white' }} >Copy</Link>
                    <Link class='btn btn-primary' to={'/profile/' + profile._id}  style={{ width: '50%', height: 'auto', fontSize: '.7rem', color: 'white'}} >Open </Link>
                </div>
            </Card.Body>
        </Card>
  );
